...
var PhysicsEngine = (function () {
  ...
  // Set friction dynamically on all balls
  function setFriction(frictionValue, frictionAirValue) {
    ...
    // Update friction on all existing balls
    balls.forEach(function(ball) {
      ball.friction = frictionValue;
      if (frictionAirValue !== undefined) {
        ball.frictionAir = frictionAirValue;
      }
    });
  ...
  function setBounciness(bouncinessValue) {
    ...
    // Update bounciness on all existing balls
    balls.forEach(function(ball) {
      ball.restitution = bouncinessValue;
    });
    
    // Update bounciness on all walls
    walls.forEach(function(wall) {
      wall.restitution = bouncinessValue;
    });
    
    console.log("Bounciness set to:", bouncinessValue, "(1.0 = perfect elasticity, no energy loss)");
    return true;
  }
