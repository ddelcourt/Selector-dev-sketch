...
var PhysicsEngine = (function () {
  ...
  var MODES = {
    ...
    zeroGravity: {
      ...
      gravity: 0,
      friction: 0.005,             // Small friction for natural spin during collisions
      frictionAir: 0,              // No air resistance (perpetual motion)
      frictionStatic: 0.005,       // Minimal static friction
      bounciness: 0.99,            // Nearly perfect elasticity (slight energy transfer to rotation)
  ...
  function init() {
    ...
    // Configure collision pairs for perfect elasticity in zero-gravity mode
      ...
      if (currentMode === 'zeroGravity') {
        ...
        if (engine.pairs && engine.pairs.list) {
          ...
          for (var i = 0; i < pairsList.length; i++) {
            ...
            // Nearly perfect elasticity (slight loss allows energy transfer to rotation)
            pair.restitution = 0.99;
    ...
    // DEBUG: Monitor collisions to diagnose bounce differences
    Events.on(engine, "collisionStart", function(event) {
      event.pairs.forEach(function(pair) {
        ...
        if (ball && wall && balls.indexOf(ball) !== -1) {
          ...
          console.log("⚫ COLLISION START - Ball #" + ballIndex + " [" + currentMode + "]:");
          console.log("  Speed before:", speed.toFixed(3));
          console.log("  Ball: restitution=" + ball.restitution + ", friction=" + ball.friction + ", frictionAir=" + ball.frictionAir);
          console.log("  Wall: restitution=" + wall.restitution + ", friction=" + wall.friction);
          console.log("  Pair: restitution=" + pair.restitution + ", friction=" + pair.friction + ", slop=" + pair.slop);
          
          // Store pre-collision speed for energy loss calculation
    ...
    Events.on(engine, "collisionEnd", function(event) {
      event.pairs.forEach(function(pair) {
        ...
        if (ball && wall && balls.indexOf(ball) !== -1) {
          ...
          var energyLoss = preSpeed ? ((preSpeed - speed) / preSpeed * 100).toFixed(1) : "?";
          
          console.log("⚫ COLLISION END - Ball #" + ballIndex + " [" + currentMode + "]:");
          console.log("  Speed after:", speed.toFixed(3), "| Energy loss:", energyLoss + "%");
          console.log("---");
          
          // Clean up pre-collision speed tracker
          preCollisionSpeeds.delete(ball.id);
        }
      });
    });
  ...
  function addBall(text, colorIndex) {
    ...
    var angularVel = (Math.random() - 0.5) * 0.0125; // Very subtle initial spin
  ...
  function setRandomVelocities(velocityRange) {
    ...
    balls.forEach(function(ball) {
      ...
      var angularVel = (Math.random() - 0.5) * 0.025;
      Body.setAngularVelocity(ball, angularVel);
    });
    ...
    balls.forEach(function(ball) {
      ...
      if (!mode.enableThreshold) {
        ...
        if (Math.abs(currentAngularVel) < 0.01) {
          ...
          Body.setAngularVelocity(ball, (Math.random() - 0.5) * 0.0125);
        }
        ...
        if (speed < 0.1) {
          ...
          var minSpeed = 0.5; // Minimum speed to prevent sticking
          Body.setVelocity(ball, { 
