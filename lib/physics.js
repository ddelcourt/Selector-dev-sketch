...
var PhysicsEngine = (function () {
  ...
  var config = {
    ...
    enableSleeping: true         // Enable Matter.js sleeping for stationary bodies
  };
  ...
  function init() {
    ...
    engine = Engine.create({
      ...
      enableSleeping: config.enableSleeping // Enable sleeping to stop stationary bodies
    });
    ...
    Events.on(engine, "afterUpdate", function () {
      balls.forEach(function (ball) {
        ...
        if (speed < config.velocityThreshold && speed > 0) {
          ...
          // Optionally put to sleep to save CPU
  ...
  return {
    ...
    setConfig: function (newConfig) {
      ...
      if (newConfig.enableSleeping !== undefined) {
        config.enableSleeping = newConfig.enableSleeping;
        if (engine) {
