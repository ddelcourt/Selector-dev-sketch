////////////////////////
//
//  Physics Engine for Bouncing Balls
//  Using Matter.js
//  ddelcourt 8.2026
//
////////////////////////

var PhysicsEngine = (function () {
  "use strict";

  // Matter.js modules
  var Engine = Matter.Engine,
    Render = Matter.Render,
    Runner = Matter.Runner,
    Bodies = Matter.Bodies,
    Composite = Matter.Composite,
    Events = Matter.Events,
    Body = Matter.Body;

  // Engine state
  var engine = null;
  var render = null;
  var runner = null;
  var balls = [];
  var walls = [];
  var isActive = false;
  var canvas = null;
  var renderEventAttached = false; // Track if event is attached
  var currentMode = 'normal'; // Track current physics mode
  var rotationCorrectionInterval = null; // Timer for zero-gravity rotation correction
  var ballRotationTargets = new Map(); // Store target angles for balls being corrected

  // Ball colors (matching your current design)
  var ballColors = [
    "rgb(242, 255, 99)", // Yellow (hidden in original)
    "rgb(99, 211, 255)", // Blue
    "rgb(229, 128, 240)", // Purple
    "rgb(255, 180, 99)", // Orange
    "rgb(99, 255, 187)", // Green
    "rgb(243, 118, 155)", // Pink
    "rgb(211, 255, 99)", // Lime
    "rgb(180, 150, 255)", // Lavender
  ];

  // Configurable physics parameters
  var config = {
    ballSizePercent: 0.06,       // 6% of screen area (aligned with defaults)
    velocityX: { min: -20, max: 20 },
    velocityY: { min: 8.5, max: 10 },
    bounciness: 1,               // Perfect elasticity by default
    friction: 0.05,
    frictionAir: 0.01,           // Air resistance
    gravity: 1.0,
    textSizeFactor: 0.22,        // Font size as proportion of ball radius
    allCaps: true,               // Whether to show text in all caps
    whiteText: false,            // Whether to use white text color
    velocityThreshold: 0.01,     // Minimum velocity before ball stops (prevents infinite micro-bouncing)
    enableWindowShake: true,     // Enable GSAP window shake effect on ball trigger
    enablePulseEffect: true,     // Enable ball trigger pulse effect
    randomTopBottom: true        // Random top/bottom ball trigger position
  };

  // Physics modes configuration
  var MODES = {
    normal: {
      name: 'Normal Gravity',
      gravity: 1.0,
      friction: 0.05,
      frictionAir: 0.01,
      bounciness: 0.95,
      velocityThreshold: 0.01,
      enableThreshold: true
    },
    reversed: {
      name: 'Reversed Gravity (Damped)',
      gravity: -0.04,              // Gentle upward float
      friction: 0.05,
      frictionAir: 0.01,
      bounciness: 0.95,
      velocityThreshold: 0.01,
      enableThreshold: true
    },
    zeroGravity: {
      name: 'Zero Gravity (Billiard Table)',
      gravity: 0,
      friction: 0.05,              // Significant friction for visible spin from collisions
      frictionAir: 0,              // No air resistance (perpetual motion)
      frictionStatic: 0.05,        // Matching static friction
      bounciness: 0.98,            // Slightly lower to allow more energy transfer to rotation
      velocityThreshold: 0,        // Never stop
      enableThreshold: false       // Never stop balls
      // Note: Friction converts linear momentum to angular momentum at contact points
    }
  };

  // Initialize the physics engine
  function init() {
    canvas = document.getElementById("physicsCanvas");
    if (!canvas) {
      console.error("Physics canvas not found");
      return false;
    }

    // Set canvas size to window size
    resizeCanvas();

    // Create engine with optimized settings for accurate collisions
    engine = Engine.create({
      gravity: { x: 0, y: config.gravity }, // Configurable gravity
      enableSleeping: false, // Disable automatic sleeping globally
      constraintIterations: 2,
      positionIterations: 10,  // Higher iterations for accurate collision resolution
      velocityIterations: 8    // Higher iterations for energy conservation
    });
    
    // Optimize for perfect elastic collisions
    engine.timing.timeScale = 1.0; // Normal time scale
    
    // Configure collision solver for billiard physics - CRITICAL for slow-velocity bounces
    // Access the Matter.Resolver module to set global collision properties
    if (Matter.Resolver) {
      // Reduce slop to absolute minimum (default is ~0.05 which causes sticking)
      Matter.Resolver._restingThresh = 0.001; // Minimum velocity threshold
      Matter.Resolver._restingThreshTangent = 0.001;
      Matter.Resolver._positionDampen = 0.9; // Position correction damping
      Matter.Resolver._positionWarming = 0.8; // Warm starting for stability
    }
    
    engine.positionIterations = 10;
    engine.velocityIterations = 8;
    
    console.log("Engine created with optimized collision settings:", engine);

    // Create renderer
    render = Render.create({
      canvas: canvas,
      engine: engine,
      options: {
        width: canvas.width,
        height: canvas.height,
        wireframes: false,
        background: "transparent",
        showSleeping: false, // Keep original colors for sleeping bodies
      },
    });

    // Create walls (floor, left wall, right wall)
    createWalls();

    // Run the engine with fixed timestep for consistent physics
    runner = Runner.create({
      isFixed: true,  // Use fixed timestep
      delta: 1000 / 60  // 60 FPS (16.667ms per frame)
    });

    // Track pre-collision speeds for energy loss analysis
    var preCollisionSpeeds = new Map();
    
    // Configure collision pairs and track velocities before physics update
    Events.on(engine, "beforeUpdate", function() {
      // Store current speeds for all balls before collision resolution (for accurate energy tracking)
      balls.forEach(function(ball) {
        if (ball && !ball.isStatic) {
          var speed = Math.sqrt(ball.velocity.x * ball.velocity.x + ball.velocity.y * ball.velocity.y);
          preCollisionSpeeds.set(ball.id, speed);
        }
      });
      
      // Configure collision pairs for zero-gravity mode
      if (currentMode === 'zeroGravity') {
        // Configure resolver for minimal resting threshold (prevent sticking)
        if (Matter.Resolver) {
          Matter.Resolver._restingThresh = 0.0001; // Nearly zero resting threshold
          Matter.Resolver._restingThreshTangent = 0.0001; // No tangential rest
        }
        
        // Configure collision pairs to prevent sticking while allowing natural rotation
        if (engine.pairs && engine.pairs.list) {
          var pairsList = engine.pairs.list;
          for (var i = 0; i < pairsList.length; i++) {
            var pair = pairsList[i];
            // High elasticity with energy transfer to rotation
            pair.restitution = 0.98;
            // Keep friction for natural rotation from collisions
            // pair.friction and pair.frictionStatic are left as-is (derived from bodies)
            
            // Force collision to always resolve (prevent sticking)
            if (pair.separation !== undefined && pair.separation < 0) {
              // If there's penetration, ensure it resolves with bounce
              pair.isActive = true;
            }
          }
        }
      }
    });
    
    // DEBUG: Monitor collisions to show energy transfer
    Events.on(engine, "collisionEnd", function(event) {
      event.pairs.forEach(function(pair) {
        var ball = null;
        var wall = null;
        
        // Identify which body is the ball and which is the wall
        if (pair.bodyA.isStatic) {
          wall = pair.bodyA;
          ball = pair.bodyB;
        } else if (pair.bodyB.isStatic) {
          wall = pair.bodyB;
          ball = pair.bodyA;
        }
        
        // Log ball-wall collisions
        if (ball && wall && balls.indexOf(ball) !== -1) {
          var ballIndex = balls.indexOf(ball);
          var speed = Math.sqrt(ball.velocity.x * ball.velocity.x + ball.velocity.y * ball.velocity.y);
          var preSpeed = preCollisionSpeeds.get(ball.id);
          
          if (preSpeed !== undefined && preSpeed > 0.1) { // Only log significant collisions
            var energyLoss = ((preSpeed - speed) / preSpeed * 100).toFixed(1);
            var angularSpeed = Math.abs(ball.angularVelocity);
            
            console.log("⚫ COLLISION - Ball #" + ballIndex + " [" + currentMode + "]:");
            console.log("  Linear speed: " + preSpeed.toFixed(3) + " → " + speed.toFixed(3) + " | Loss: " + energyLoss + "%");
            console.log("  Angular speed: " + angularSpeed.toFixed(4) + " rad/s");
            console.log("  Pair restitution: " + pair.restitution.toFixed(3) + ", friction: " + pair.friction.toFixed(4));
          }
        }
      });
    });

    // Velocity management: cap max speed and apply threshold to stop slow balls
    Events.on(engine, "afterUpdate", function () {
      var mode = MODES[currentMode];
      
      balls.forEach(function (ball) {
        if (!ball || ball.isStatic) return;
        
        var velocity = ball.velocity;
        var speed = Math.sqrt(velocity.x * velocity.x + velocity.y * velocity.y);
        
        // Only apply velocity threshold in modes where it's enabled (not in zero-gravity)
        if (mode.enableThreshold && speed < mode.velocityThreshold && speed > 0) {
          Body.setVelocity(ball, { x: 0, y: 0 });
          Body.setAngularVelocity(ball, 0);
        }
        
        // In zero-gravity mode, enforce minimum velocity to prevent complete stop
        if (!mode.enableThreshold && speed > 0 && speed < 0.3) {
          // Gently boost velocity to minimum threshold while preserving direction
          var minSpeed = 0.3;
          var scale = minSpeed / speed;
          Body.setVelocity(ball, { 
            x: velocity.x * scale, 
            y: velocity.y * scale 
          });
        }
        
        // Cap maximum velocity ONLY to prevent wall tunneling (higher cap for zero-gravity)
        var maxSpeed = mode.enableThreshold ? 20 : 50;
        if (speed > maxSpeed) {
          var scale = maxSpeed / speed;
          Body.setVelocity(ball, { 
            x: velocity.x * scale, 
            y: velocity.y * scale 
          });
        }
      });
      
      // Apply gradual rotation corrections (zero-gravity mode)
      if (ballRotationTargets.size > 0) {
        ballRotationTargets.forEach(function(correction, ball) {
          // Increment progress (0 to 1 over ~1 second at 60fps = 60 frames)
          correction.progress += 1 / 60;
          
          if (correction.progress >= 1) {
            // Correction complete - set final angle and remove from map
            Body.setAngle(ball, correction.targetAngle);
            ballRotationTargets.delete(ball);
            console.log("Rotation correction complete for ball");
          } else {
            // Interpolate angle using easing (smooth start and end)
            var t = correction.progress;
            var easedT = t < 0.5 ? 2 * t * t : -1 + (4 - 2 * t) * t; // Ease in-out quad
            
            // Calculate shortest path between start and target
            var deltaAngle = correction.targetAngle - correction.startAngle;
            while (deltaAngle > Math.PI) deltaAngle -= 2 * Math.PI;
            while (deltaAngle < -Math.PI) deltaAngle += 2 * Math.PI;
            
            // Interpolate and set the angle
            var newAngle = correction.startAngle + (deltaAngle * easedT);
            Body.setAngle(ball, newAngle);
          }
        });
      }
      
      // Check falling balls and remove them when off-screen
      if (fallingBalls.length > 0) {
        var canvasHeight = canvas ? canvas.height : window.innerHeight;
        var ballsToRemove = [];
        
        fallingBalls.forEach(function(ball) {
          // Check if ball has fallen below the screen (with some margin)
          if (ball.position.y > canvasHeight + ball.ballRadius * 2) {
            ballsToRemove.push(ball);
          }
        });
        
        // Remove balls that have fallen off-screen
        ballsToRemove.forEach(function(ball) {
          Composite.remove(engine.world, ball);
          var index = fallingBalls.indexOf(ball);
          if (index > -1) {
            fallingBalls.splice(index, 1);
          }
          console.log("Removed fallen ball from world. Falling balls remaining:", fallingBalls.length);
        });
      }
    });

    // Set up custom rendering for text labels (only once)
    if (!renderEventAttached) {
      Events.on(render, "afterRender", function () {
        var context = render.context;
        
        // Combine regular balls and falling balls for rendering
        var allBalls = balls.concat(fallingBalls);
        
        // Ensure we have balls to render
        if (!allBalls || allBalls.length === 0) return;
        
        allBalls.forEach(function (ball) {
          // Skip if ball doesn't have required properties
          if (!ball || !ball.label || !ball.position || !ball.ballRadius) return;

          var pos = ball.position;
          var radius = ball.ballRadius;

          // Calculate proportional font size
          var fontSize = Math.max(10, Math.round(radius * config.textSizeFactor));
          var lineHeight = fontSize * 1.2;

          // Draw text centered in ball
          context.save();
          context.translate(pos.x, pos.y);
          context.rotate(ball.angle);

          context.fillStyle = config.whiteText ? "#fff" : "#000";
          context.font = "bold " + fontSize + "px Arial, sans-serif";
          context.textAlign = "center";
          context.textBaseline = "middle";

          // Convert label to string and word wrap
          var labelText = String(ball.label);
          
          // Apply all caps if enabled
          if (config.allCaps) {
            labelText = labelText.toUpperCase();
          }
          
          var words = labelText.split(" ");
          var lines = [];
          var currentLine = words[0] || "";

          for (var i = 1; i < words.length; i++) {
            var testLine = currentLine + " " + words[i];
            var metrics = context.measureText(testLine);
            if (metrics.width > radius * 1.5) {
              lines.push(currentLine);
              currentLine = words[i];
            } else {
              currentLine = testLine;
            }
          }
          lines.push(currentLine);

          // Draw lines
          var totalHeight = lines.length * lineHeight;
          var startY = -totalHeight / 2 + lineHeight / 2;

          lines.forEach(function (line, idx) {
            context.fillText(line, 0, startY + idx * lineHeight);
          });

          context.restore();
        });
      });
      renderEventAttached = true;
      console.log("Render event handler attached");
    }

    console.log("Physics engine initialized");
    return true;
  }

  // Rescale all balls by a given factor
  function rescaleBalls(scaleFactor) {
    if (!balls || balls.length === 0) return;
    if (scaleFactor === 1) return; // No change needed
    
    console.log("Rescaling", balls.length, "balls by factor:", scaleFactor.toFixed(3));
    
    balls.forEach(function(ball) {
      if (!ball || !ball.ballRadius) return;
      
      // Scale the Matter.js body
      Body.scale(ball, scaleFactor, scaleFactor);
      
      // Update stored radius
      ball.ballRadius = ball.ballRadius * scaleFactor;
      
      // Note: Text size will automatically adjust based on the new ballRadius
      // since it's calculated proportionally in the render loop
    });
    
    // Also rescale falling balls
    if (fallingBalls && fallingBalls.length > 0) {
      fallingBalls.forEach(function(ball) {
        if (!ball || !ball.ballRadius) return;
        Body.scale(ball, scaleFactor, scaleFactor);
        ball.ballRadius = ball.ballRadius * scaleFactor;
      });
    }
  }

  // Resize canvas to match window size
  function resizeCanvas() {
    if (!canvas) return;
    
    var oldWidth = canvas.width;
    var oldHeight = canvas.height;
    
    // Get the actual rendered canvas dimensions from CSS
    var rect = canvas.getBoundingClientRect();
    var width = rect.width;
    var height = rect.height;
    
    // If dimensions are 0, fall back to viewport dimensions
    if (width === 0 || height === 0) {
      width = document.documentElement.clientWidth || window.innerWidth;
      height = document.documentElement.clientHeight || window.innerHeight;
    }
    
    canvas.width = width;
    canvas.height = height;
    
    console.log("Canvas resized from:", oldWidth, "x", oldHeight, "to:", width, "x", height);
    
    if (render) {
      render.options.width = canvas.width;
      render.options.height = canvas.height;
      render.canvas.width = canvas.width;
      render.canvas.height = canvas.height;
      
      // Calculate scale factor based on screen area change
      var oldArea = oldWidth * oldHeight;
      var newArea = width * height;
      var areaScale = newArea / oldArea;
      var radiusScale = Math.sqrt(areaScale); // Scale radius by sqrt of area change
      
      console.log("Area scale:", areaScale.toFixed(3), "→ Radius scale:", radiusScale.toFixed(3));
      
      // Rescale all existing balls
      rescaleBalls(radiusScale);
      
      // Adjust ball positions if they're outside new boundaries
      adjustBallPositions(oldWidth, oldHeight, width, height);
      
      // Recreate walls with new dimensions
      removeWalls();
      createWalls();
    }
  }
  
  // Adjust ball positions when canvas is resized
  function adjustBallPositions(oldWidth, oldHeight, newWidth, newHeight) {
    if (!balls || balls.length === 0) return;
    
    var wallThickness = 200; // Match the wall thickness in createWalls()
    var minX = 10; // Inner edge is at 0, so just add small margin
    var maxX = newWidth - 10;
    var minY = 10;
    var maxY = newHeight - 10;
    
    console.log("Adjusting ball positions for new canvas size");
    
    balls.forEach(function(ball) {
      if (!ball || !ball.ballRadius) return;
      
      var radius = ball.ballRadius;
      var pos = ball.position;
      var newX = pos.x;
      var newY = pos.y;
      var adjusted = false;
      
      // Check if ball is outside new boundaries and constrain position
      if (pos.x - radius < minX) {
        newX = minX + radius;
        adjusted = true;
      } else if (pos.x + radius > maxX) {
        newX = maxX - radius;
        adjusted = true;
      }
      
      if (pos.y - radius < minY) {
        newY = minY + radius;
        adjusted = true;
      } else if (pos.y + radius > maxY) {
        newY = maxY - radius;
        adjusted = true;
      }
      
      // If ball needed adjustment, set new position
      if (adjusted) {
        Body.setPosition(ball, { x: newX, y: newY });
        // Only dampen velocity in non-zero-gravity modes to prevent extreme bouncing
        // In zero-gravity mode, preserve velocity for perpetual motion
        var mode = MODES[currentMode];
        if (mode.enableThreshold) {
          Body.setVelocity(ball, { 
            x: ball.velocity.x * 0.5, 
            y: ball.velocity.y * 0.5 
          });
        }
        console.log("Adjusted ball position from", pos.x.toFixed(0), ",", pos.y.toFixed(0), "to", newX.toFixed(0), ",", newY.toFixed(0));
      }
    });
  }

  // Create boundary walls
  function createWalls() {
    var wallThickness = 200; // Thick walls to prevent fast balls from tunneling/deep penetration
    var w = canvas.width;
    var h = canvas.height;

    // Floor - positioned so inner surface is at canvas bottom (y = h)
    var floor = Bodies.rectangle(w / 2, h + wallThickness / 2, w + wallThickness * 2, wallThickness, {
      isStatic: true,
      restitution: config.bounciness, // Use configurable bounciness
      friction: config.friction,
      render: { fillStyle: "transparent" },
    });

    // Left wall - positioned so inner surface is at canvas left edge (x = 0)
    var leftWall = Bodies.rectangle(-wallThickness / 2, h / 2, wallThickness, h + wallThickness * 2, {
      isStatic: true,
      restitution: config.bounciness,
      friction: config.friction,
      render: { fillStyle: "transparent" },
    });

    // Right wall - positioned so inner surface is at canvas right edge (x = w)
    var rightWall = Bodies.rectangle(w + wallThickness / 2, h / 2, wallThickness, h + wallThickness * 2, {
      isStatic: true,
      restitution: config.bounciness,
      friction: config.friction,
      render: { fillStyle: "transparent" },
    });

    // Ceiling - positioned so inner surface is at canvas top (y = 0)
    var ceiling = Bodies.rectangle(w / 2, -wallThickness / 2, w + wallThickness * 2, wallThickness, {
      isStatic: true,
      restitution: config.bounciness,
      friction: config.friction,
      render: { fillStyle: "transparent" },
    });

    walls = [floor, leftWall, rightWall, ceiling];
    Composite.add(engine.world, walls);
    
    console.log("Walls created: floor y=" + floor.position.y + ", ceiling y=" + ceiling.position.y + 
                ", left x=" + leftWall.position.x + ", right x=" + rightWall.position.x);
  }

  // Remove walls from the world
  function removeWalls() {
    walls.forEach(function (wall) {
      Composite.remove(engine.world, wall);
    });
    walls = [];
  }

  // Create pulse effect at specified position
  function createPulseEffect(x, y, color) {
    // Check if GSAP is available
    if (typeof gsap === 'undefined') {
      console.warn('GSAP not loaded, skipping pulse effect');
      return;
    }
    
    var pulseContainer = document.getElementById('pulseContainer');
    if (!pulseContainer) return;
    
    // Create pulse element
    var pulse = document.createElement('div');
    pulse.className = 'pulse-circle';
    pulse.style.left = x + 'px';
    pulse.style.top = y + 'px';
    pulse.style.width = '0px';
    pulse.style.height = '0px';
    pulse.style.backgroundColor = color || 'rgba(255, 255, 255, 0.9)';
    pulse.style.opacity = '0.25';
    
    pulseContainer.appendChild(pulse);
    
    // Calculate size larger than viewport
    var maxDimension = Math.max(window.innerWidth, window.innerHeight);
    var pulseSize = maxDimension * 2.5; // 2.5x larger than viewport
    
    // Animate pulse using GSAP - slower and bigger
    gsap.to(pulse, {
      width: pulseSize,
      height: pulseSize,
      opacity: 0,
      duration: 1.8,
      ease: 'power1.out',
      onComplete: function() {
        if (pulse.parentNode) {
          pulseContainer.removeChild(pulse);
        }
      }
    });
  }
  
  // Create screen shake effect
  function createScreenShake() {
    // Check if GSAP is available
    if (typeof gsap === 'undefined') {
      console.warn('GSAP not loaded, skipping shake effect');
      return;
    }
    
    // Target the canvas instead of body to avoid layout issues
    var target = canvas || document.getElementById('physicsCanvas');
    if (!target) return;
    
    // Subtle shake animation
    gsap.to(target, {
      x: '+=3',
      y: '+=3',
      duration: 0.05,
      ease: 'power1.inOut',
      yoyo: true,
      repeat: 5,
      onComplete: function() {
        gsap.set(target, { x: 0, y: 0 });
      }
    });
  }

  // Calculate ball radius based on screen area
  function calculateBallRadius(percentage) {
    // Use config value if no percentage provided
    var pct = percentage !== undefined ? percentage : config.ballSizePercent;
    

    
    // Calculate screen area
    var screenArea = window.innerWidth * window.innerHeight;
    
    // Calculate base radius from screen area
    // radius = sqrt(area * percentage / PI) to get radius from area
    var baseRadius = Math.sqrt((screenArea * pct) / Math.PI);
    
    // Add some randomness (±20%)
    var randomFactor = 0.8 + Math.random() * 0.4;
    var radius = baseRadius * randomFactor;
    
    // Clamp between reasonable values (increased max for larger balls)
    var minRadius = 30;
    var maxRadius = 800;
    radius = Math.max(minRadius, Math.min(maxRadius, radius));
    

    
    return radius;
  }

  // Add a new ball to the simulation
  function addBall(text, colorIndex) {
    if (!engine || !isActive) {
      console.warn("Cannot add ball: engine not active");
      return null;
    }

    // Convert text to string to ensure it's valid
    var labelText = text ? String(text) : "?";


    var radius = calculateBallRadius(); // Calculate size based on screen area
    var x = Math.random() * (canvas.width - 200) + 100; // Random X position
    
    // Position logic based on config
    var launchFromBelow = false;
    if (config.randomTopBottom) {
      // 33% chance to launch from below, 67% chance to drop from top
      launchFromBelow = Math.random() < 0.33;
    }
    var y = launchFromBelow ? canvas.height + 100 : 100; // Below screen or near top

    // Random initial velocity using config values
    var vx = config.velocityX.min + Math.random() * (config.velocityX.max - config.velocityX.min);
    var vy = config.velocityY.min + Math.random() * (config.velocityY.max - config.velocityY.min);
    
    // If launching from below, reverse velocity to shoot upward
    if (launchFromBelow) {
      vy = -vy; // Negative velocity = upward motion
    }

    // Get color for this ball
    var colorIdx = (colorIndex !== undefined ? colorIndex : balls.length) % ballColors.length;
    var color = ballColors[colorIdx];

    // Get friction values from current mode if set, otherwise use config
    var mode = MODES[currentMode] || MODES['normal'];
    var ballFriction = mode.friction !== undefined ? mode.friction : config.friction;
    var ballFrictionAir = mode.frictionAir !== undefined ? mode.frictionAir : (config.frictionAir || 0.01);
    var ballFrictionStatic = mode.frictionStatic !== undefined ? mode.frictionStatic : ballFriction;

    // Create ball body with consistent density for uniform mass distribution
    var ball = Bodies.circle(x, y, radius, {
      restitution: config.bounciness, // Use configurable bounciness
      friction: ballFriction,
      frictionAir: ballFrictionAir,
      frictionStatic: ballFrictionStatic,
      density: 0.001, // Consistent density for all balls (mass = density * area)
      sleepThreshold: Infinity, // Never sleep automatically
      render: {
        fillStyle: color,
      },
    });
    
    // Note: Matter.js does not have an 'angularFriction' property
    // Angular velocity damping is controlled through the world's timing properties
    // Balls naturally rotate based on their inertia (calculated from mass and radius)

    // Store custom data
    ball.label = labelText;
    ball.ballColor = color;
    ball.ballRadius = radius;

    // Set initial velocity
    Body.setVelocity(ball, { x: vx, y: vy });
    
    // Add small initial angular velocity for visual interest
    // Balls will gain more natural rotation through collisions via friction
    var angularVel = (Math.random() - 0.5) * 0.00625; // Very subtle initial spin
    Body.setAngularVelocity(ball, angularVel);

    // Add to world
    Composite.add(engine.world, ball);
    balls.push(ball);

    // Trigger visual feedback effects (if enabled)
    if (config.enablePulseEffect) {
      createPulseEffect(x, y, color);
    }
    if (config.enableWindowShake) {
      createScreenShake();
    }

    return ball;
  }

  // Clear all balls
  function clearBalls() {
    console.log("Clearing", balls.length, "balls");
    balls.forEach(function (ball) {
      Composite.remove(engine.world, ball);
    });
    balls = [];
  }

  // Track balls that are falling through the floor
  var fallingBalls = [];

  // Remove the last ball added
  function removeLastBall() {
    if (balls.length === 0) {
      console.warn("No balls to remove");
      return null;
    }
    
    var lastBall = balls.pop();
    
    // Check if we're in normal gravity mode
    if (currentMode === 'normal') {
      // Make the ball fall through the floor with dramatic effect
      console.log("Ball falling through floor (sin-hole effect)");
      
      // Mark ball as falling (will be removed when off-screen)
      lastBall.isFalling = true;
      fallingBalls.push(lastBall);
      
      // Disable collisions with walls by changing collision filter
      // Set to a unique collision group that doesn't interact with walls
      Body.set(lastBall, {
        collisionFilter: {
          category: 0x0002,  // Different category
          mask: 0x0000       // Doesn't collide with anything
        }
      });
      
      // Remove any existing velocity
      Body.setVelocity(lastBall, { x: 0, y: 0 });
      
      // Apply strong downward force to simulate falling into hole
      // Using a larger gravity multiplier for dramatic effect
      Body.applyForce(lastBall, lastBall.position, { 
        x: 0, 
        y: lastBall.mass * 0.02  // Strong downward force
      });
      
      // Reduce friction and air resistance for smooth falling
      Body.set(lastBall, {
        friction: 0,
        frictionAir: 0.001,
        restitution: 0  // No bouncing when falling
      });
      
      console.log("Ball will be removed after falling off-screen. Remaining balls:", balls.length);
    } else {
      // In zero-gravity or reversed mode, remove immediately
      Composite.remove(engine.world, lastBall);
      console.log("Removed last ball immediately (non-normal mode). Remaining balls:", balls.length);
    }
    
    return lastBall;
  }

  // Start the physics simulation
  function start() {
    if (!engine) {
      if (!init()) {
        console.error("Failed to initialize engine");
        return false;
      }
    }

    isActive = true;
    canvas.style.display = "block";

    Render.run(render);
    Runner.run(runner, engine);

    return true;
  }

  // Stop the physics simulation
  function stop() {
    isActive = false;
    if (canvas) {
      canvas.style.display = "none";
    }

    if (render) {
      Render.stop(render);
    }

    if (runner && engine) {
      Runner.stop(runner);
    }
    
    // Clear rotation correction interval
    if (rotationCorrectionInterval) {
      clearInterval(rotationCorrectionInterval);
      rotationCorrectionInterval = null;
      console.log("Cleared rotation correction interval on stop");
    }
    
    // Clear rotation correction targets
    ballRotationTargets.clear();
  }

  // Set gravity dynamically
  function setGravity(gravityValue) {
    if (!engine) {
      console.warn("Cannot set gravity: engine not initialized");
      return false;
    }
    
    config.gravity = gravityValue;
    engine.gravity.y = gravityValue;
    return true;
  }

  // Set friction dynamically on all balls and walls
  function setFriction(frictionValue, frictionAirValue) {
    if (!engine) {
      console.warn("Cannot set friction: engine not initialized");
      return false;
    }
    
    // Allow setting even if no balls yet (will apply to future balls)
    config.friction = frictionValue;
    
    // If no balls yet, just update config and return
    if (balls.length === 0) {
      return true;
    }
    
    // Update friction on all existing balls using Body.set for proper updates
    var ballCount = 0;
    balls.forEach(function(ball) {
      Body.set(ball, {
        friction: frictionValue,
        frictionStatic: frictionValue,
        frictionAir: frictionAirValue !== undefined ? frictionAirValue : ball.frictionAir
      });
      ballCount++;
    });
    
    // Update friction on all walls using Body.set
    var wallCount = 0;
    walls.forEach(function(wall) {
      Body.set(wall, {
        friction: frictionValue,
        frictionStatic: frictionValue
      });
      wallCount++;
    });
    
    return true;
  }

  // Set bounciness (restitution) dynamically on all balls and walls
  function setBounciness(bouncinessValue) {
    if (!engine) {
      console.warn("Cannot set bounciness: engine not initialized");
      return false;
    }
    
    config.bounciness = bouncinessValue;
    
    // Update bounciness on all existing balls using Body.set
    balls.forEach(function(ball) {
      Body.set(ball, {
        restitution: bouncinessValue
      });
    });
    
    // Update bounciness on all walls using Body.set
    if (walls && walls.length > 0) {
      walls.forEach(function(wall) {
        Body.set(wall, {
          restitution: bouncinessValue
        });
      });
      console.log("Bounciness set to:", bouncinessValue, "on", balls.length, "balls and", walls.length, "walls");
    } else {
      console.warn("No walls found - only updated balls. Walls array:", walls);
      console.log("Bounciness set to:", bouncinessValue, "on", balls.length, "balls only");
    }
    
    return true;
  }

  // Set random velocities on all balls (for zero-gravity scatter effect)
  function setRandomVelocities(velocityRange) {
    if (!engine || balls.length === 0) {
      console.warn("Cannot set random velocities: engine not initialized or no balls");
      return false;
    }
    
    // Default velocity range if not provided
    var vRange = velocityRange || { min: -3, max: 3 };
    
    balls.forEach(function(ball) {
      var vx = vRange.min + Math.random() * (vRange.max - vRange.min);
      var vy = vRange.min + Math.random() * (vRange.max - vRange.min);
      Body.setVelocity(ball, { x: vx, y: vy });
      
      // Add subtle random angular velocity - balls will gain natural spin from collisions
      var angularVel = (Math.random() - 0.5) * 0.0125;
      Body.setAngularVelocity(ball, angularVel);
    });
    
    console.log("Random velocities and spins applied to", balls.length, "balls");
    return true;
  }

  // Correct ball rotation to keep text readable (zero-gravity mode)
  function correctBallRotation() {
    if (!engine || balls.length === 0) return;
    
    // Select a random ball
    var randomIndex = Math.floor(Math.random() * balls.length);
    var ball = balls[randomIndex];
    
    // Convert angle from radians to degrees
    var angleDegrees = (ball.angle * 180) / Math.PI;
    
    // Normalize angle to -180 to 180 range
    while (angleDegrees > 180) angleDegrees -= 360;
    while (angleDegrees < -180) angleDegrees += 360;
    
    // Check if angle is outside the ±60° range
    if (angleDegrees < -60 || angleDegrees > 60) {
      // Choose a random target angle within ±60°
      var targetDegrees = -60 + Math.random() * 120; // Random angle between -60 and 60
      var targetRadians = (targetDegrees * Math.PI) / 180;
      
      // Store the target angle for this ball (will be applied gradually in afterUpdate)
      ballRotationTargets.set(ball, {
        targetAngle: targetRadians,
        startAngle: ball.angle,
        progress: 0
      });
      
      console.log("Marking ball for rotation correction from " + angleDegrees.toFixed(1) + "° to " + targetDegrees.toFixed(1) + "°");
    }
  }

  // Set physics mode (normal, reversed, zeroGravity)
  function setMode(modeName, options) {
    if (!MODES[modeName]) {
      console.error("Unknown physics mode:", modeName);
      return false;
    }
    
    if (!engine) {
      console.warn("Cannot set mode: engine not initialized");
      return false;
    }
    
    var mode = MODES[modeName];
    currentMode = modeName;
    
    console.log("Setting physics mode to:", mode.name);
    
    // Apply mode settings
    setGravity(mode.gravity);
    setBounciness(mode.bounciness);
    
    // Update config for new balls
    config.gravity = mode.gravity;
    config.friction = mode.friction;
    config.frictionAir = mode.frictionAir;
    config.bounciness = mode.bounciness;
    config.velocityThreshold = mode.velocityThreshold;
    
    // Manage rotation correction interval for zero-gravity mode
    if (modeName === 'zeroGravity') {
      // Start rotation correction interval if not already running
      if (!rotationCorrectionInterval) {
        rotationCorrectionInterval = setInterval(correctBallRotation, 3000); // Every 3 seconds
        console.log("Started rotation correction interval for zero-gravity mode");
      }
    } else {
      // Stop rotation correction interval when leaving zero-gravity mode
      if (rotationCorrectionInterval) {
        clearInterval(rotationCorrectionInterval);
        rotationCorrectionInterval = null;
        ballRotationTargets.clear(); // Clear any pending corrections
        console.log("Stopped rotation correction interval");
      }
    }
    
    // Configure engine solver based on mode
    if (modeName === 'zeroGravity') {
      // High precision solver for perfect elastic collisions
      engine.positionIterations = 12;
      engine.velocityIterations = 10;
      engine.constraintIterations = 3;
      
      // Configure Matter.Resolver for zero-gravity billiard physics
      if (Matter.Resolver) {
        Matter.Resolver._restingThresh = 0.0001; // Nearly zero - balls never "rest" against walls
        Matter.Resolver._restingThreshTangent = 0.0001;
        Matter.Resolver._positionDampen = 0.9;
        Matter.Resolver._positionWarming = 0.8;
      }
      
      console.log("Configured high-precision solver for zero-gravity mode");
    } else {
      // Normal solver settings
      engine.positionIterations = 10;
      engine.velocityIterations = 8;
      engine.constraintIterations = 2;
      
      // Restore normal resting thresholds
      if (Matter.Resolver) {
        Matter.Resolver._restingThresh = 0.001;
        Matter.Resolver._restingThreshTangent = 0.001;
      }
    }
    
    // Apply friction settings to all balls
    balls.forEach(function(ball) {
      Body.set(ball, {
        friction: mode.friction,
        frictionStatic: mode.frictionStatic !== undefined ? mode.frictionStatic : mode.friction,
        frictionAir: mode.frictionAir,
        restitution: mode.bounciness, // Ensure restitution is updated
        isSleeping: false,
        sleepThreshold: Infinity,
        timeScale: 1.0
      });
      
      // Wake up and ensure motion in zero-gravity mode
      if (!mode.enableThreshold) {
        // Force ball to be fully awake
        ball.isSleeping = false;
        ball.sleepCounter = 0;
        
        // Add subtle angular velocity if not spinning - natural rotation from collisions will dominate
        var currentAngularVel = ball.angularVelocity || 0;
        if (Math.abs(currentAngularVel) < 0.01) {
          // Add small random spin to start
          Body.setAngularVelocity(ball, (Math.random() - 0.5) * 0.00625);
        }
        
        // Give tiny nudge if ball is stationary or moving very slowly
        var speed = Math.sqrt(ball.velocity.x * ball.velocity.x + ball.velocity.y * ball.velocity.y);
        if (speed < 0.3) {
          var angle = Math.random() * Math.PI * 2;
          var minSpeed = 0.3; // Minimum speed to prevent stopping (matches afterUpdate check)
          Body.setVelocity(ball, { 
            x: Math.cos(angle) * minSpeed, 
            y: Math.sin(angle) * minSpeed 
          });
        }
      }
    });
    
    // Apply friction and restitution settings to all walls
    walls.forEach(function(wall) {
      Body.set(wall, {
        friction: mode.friction,
        frictionStatic: mode.frictionStatic !== undefined ? mode.frictionStatic : mode.friction,
        restitution: mode.bounciness
      });
    });
    
    // In zero-gravity mode, force all pairs to be active (no resting)
    if (modeName === 'zeroGravity' && engine.pairs && engine.pairs.list) {
      var pairsList = engine.pairs.list;
      for (var i = 0; i < pairsList.length; i++) {
        var pair = pairsList[i];
        // Ensure pairs stay active even at low velocities
        pair.isActive = true;
      }
    }
    
    // Configure engine sleeping
    if (engine) {
      engine.enableSleeping = mode.enableThreshold;
    }
    
    // Apply random velocities if options provided
    if (options && options.randomVelocities) {
      setRandomVelocities(options.randomVelocities);
    }
    
    console.log("Physics mode set to:", modeName, "- Gravity:", mode.gravity, "Friction:", mode.friction, "Bounciness:", mode.bounciness);
    console.log("  Updated", balls.length, "balls and", walls.length, "walls");
    return true;
  }

  // Get current mode name
  function getMode() {
    return currentMode;
  }

  // Enable/disable velocity threshold (deprecated - use setMode instead)
  function setVelocityThreshold(enabled) {
    console.warn("setVelocityThreshold is deprecated. Use setMode() instead.");
    // For backward compatibility, switch to appropriate mode
    if (enabled) {
      setMode('normal');
    } else {
      setMode('zeroGravity');
    }
    return true;
  }

  // Diagnostic: Print all physics properties of first ball and wall
  function diagnosticCheck() {
    if (balls.length === 0 || walls.length === 0) {
      console.log("DIAGNOSTIC: No balls or walls to check");
      console.log("  - balls.length:", balls.length);
      console.log("  - walls.length:", walls.length);
      console.log("  - engine.world.bodies:", engine ? engine.world.bodies.length : "no engine");
      return;
    }
    
    var ball = balls[0];
    var wall = walls[0];
    
    console.log("=== DIAGNOSTIC CHECK ===");
    console.log("Ball properties:");
    console.log("  - friction:", ball.friction);
    console.log("  - frictionStatic:", ball.frictionStatic);
    console.log("  - frictionAir:", ball.frictionAir);
    console.log("  - restitution:", ball.restitution);
    console.log("  - sleepThreshold:", ball.sleepThreshold);
    console.log("  - isSleeping:", ball.isSleeping);
    console.log("  - density:", ball.density);
    console.log("  - mass:", ball.mass);
    console.log("  - inertia:", ball.inertia);
    console.log("  - velocity:", "x=" + ball.velocity.x.toFixed(3), "y=" + ball.velocity.y.toFixed(3));
    console.log("  - speed:", Math.sqrt(ball.velocity.x**2 + ball.velocity.y**2).toFixed(3));
    console.log("  - angularVelocity:", ball.angularVelocity.toFixed(3));
    console.log("  - angle:", (ball.angle * 180 / Math.PI).toFixed(1), "degrees");
    
    console.log("Wall properties:");
    console.log("  - friction:", wall.friction);
    console.log("  - frictionStatic:", wall.frictionStatic);
    console.log("  - restitution:", wall.restitution);
    console.log("  - isStatic:", wall.isStatic);
    
    console.log("Engine properties:");
    console.log("  - gravity.y:", engine.gravity.y);
    console.log("  - enableSleeping:", engine.enableSleeping);
    console.log("  - positionIterations:", engine.positionIterations);
    console.log("  - velocityIterations:", engine.velocityIterations);
    console.log("  - constraintIterations:", engine.constraintIterations);
    console.log("  - timing.timeScale:", engine.timing.timeScale);
    
    console.log("State:");
    console.log("  - currentMode:", currentMode);
    console.log("  - Total bodies in world:", engine.world.bodies.length);
    console.log("  - Walls in world:", walls.filter(function(w) { 
        return engine.world.bodies.indexOf(w) !== -1; 
      }).length);
    
    // Calculate total energy in system
    var totalKE = 0;
    var totalAngularKE = 0;
    balls.forEach(function(b) {
      var speed = Math.sqrt(b.velocity.x**2 + b.velocity.y**2);
      totalKE += b.mass * speed * speed / 2;
      totalAngularKE += b.inertia * b.angularVelocity * b.angularVelocity / 2;
    });
    console.log("Total kinetic energy:", totalKE.toFixed(3));
    console.log("Total angular energy:", totalAngularKE.toFixed(3));
    console.log("======================");
  }

  // Monitor energy levels over time
  function monitorEnergy(intervalMs) {
    var monitoringInterval = setInterval(function() {
      if (!isActive || balls.length === 0) {
        clearInterval(monitoringInterval);
        return;
      }
      
      var totalKE = 0;
      var totalAngularKE = 0;
      balls.forEach(function(b) {
        var speed = Math.sqrt(b.velocity.x**2 + b.velocity.y**2);
        totalKE += b.mass * speed * speed / 2;
        totalAngularKE += b.inertia * b.angularVelocity * b.angularVelocity / 2;
      });
      

    }, intervalMs || 2000);
    
    return monitoringInterval;
  }

  // Public API
  return {
    init: init,
    start: start,
    stop: stop,
    addBall: addBall,
    clearBalls: clearBalls,
    removeLastBall: removeLastBall,
    resizeCanvas: resizeCanvas,
    
    // Mode management (recommended API)
    setMode: setMode,
    getMode: getMode,
    
    // Direct property setters (for advanced use)
    setGravity: setGravity,
    setFriction: setFriction,
    setBounciness: setBounciness,
    setRandomVelocities: setRandomVelocities,
    setVelocityThreshold: setVelocityThreshold, // Deprecated, kept for compatibility
    
    // Debug utilities
    diagnosticCheck: diagnosticCheck,
    monitorEnergy: monitorEnergy,
    
    isActive: function () {
      return isActive;
    },
    getBalls: function () {
      return balls; // Return reference for debugging
    },
    // Config getters and setters
    getConfig: function () {
      return Object.assign({}, config); // Return a copy
    },
    setConfig: function (newConfig) {
      // Store old values for comparison
      var oldBallSize = config.ballSizePercent;
      var oldBounciness = config.bounciness;
      var oldFriction = config.friction;
      
      console.log("setConfig called with:", newConfig);
      console.log("Current config before update:", config);
      
      // Update config values
      if (newConfig.ballSizePercent !== undefined) config.ballSizePercent = newConfig.ballSizePercent;
      if (newConfig.velocityX !== undefined) config.velocityX = newConfig.velocityX;
      if (newConfig.velocityY !== undefined) config.velocityY = newConfig.velocityY;
      if (newConfig.bounciness !== undefined) config.bounciness = newConfig.bounciness;
      if (newConfig.friction !== undefined) config.friction = newConfig.friction;
      if (newConfig.textSizeFactor !== undefined) config.textSizeFactor = newConfig.textSizeFactor;
      if (newConfig.allCaps !== undefined) config.allCaps = newConfig.allCaps;
      if (newConfig.whiteText !== undefined) config.whiteText = newConfig.whiteText;
      if (newConfig.velocityThreshold !== undefined) config.velocityThreshold = newConfig.velocityThreshold;
      if (newConfig.enableWindowShake !== undefined) config.enableWindowShake = newConfig.enableWindowShake;
      if (newConfig.enablePulseEffect !== undefined) config.enablePulseEffect = newConfig.enablePulseEffect;
      if (newConfig.randomTopBottom !== undefined) config.randomTopBottom = newConfig.randomTopBottom;
      if (newConfig.gravity !== undefined) {
        config.gravity = newConfig.gravity;
        // Update engine gravity if it exists
        if (engine) {
          engine.gravity.y = config.gravity;
        }
      }
      
      console.log("Config after update:", config);
      console.log("Specifically, config.ballSizePercent is now:", config.ballSizePercent);
      console.log("Old ball size:", oldBallSize, "New ball size:", config.ballSizePercent);
      console.log("Active:", isActive, "Balls count:", balls.length);
      
      // Update existing balls with new properties
      if (isActive && balls.length > 0) {
        // Update ball size if changed
        if (newConfig.ballSizePercent !== undefined && oldBallSize !== config.ballSizePercent) {
          var scaleFactor = Math.sqrt(config.ballSizePercent / oldBallSize);
          console.log("Scaling balls by factor:", scaleFactor, "from", oldBallSize, "to", config.ballSizePercent);
          balls.forEach(function(ball) {
            var oldRadius = ball.ballRadius;
            Body.scale(ball, scaleFactor, scaleFactor);
            // Update stored radius to match scaled size
            ball.ballRadius = ball.ballRadius * scaleFactor;
            console.log("Scaled ball from radius", oldRadius, "to", ball.ballRadius, "actual body vertices:", ball.vertices.length);
          });
          console.log("Scaled existing balls by factor:", scaleFactor);
        }
        
        // Update bounciness and friction on existing balls
        if (newConfig.bounciness !== undefined || newConfig.friction !== undefined) {
          balls.forEach(function(ball) {
            if (newConfig.bounciness !== undefined) {
              ball.restitution = config.bounciness;
            }
            if (newConfig.friction !== undefined) {
              ball.friction = config.friction;
            }
          });
          console.log("Updated existing balls physics properties");
        }
      }
      
      // If engine is active, recreate walls with new friction/bounciness
      if (isActive && engine) {
        removeWalls();
        createWalls();
      }
      
      console.log("Config updated:", config);
    }
  };
})();

// Make PhysicsEngine available globally for ES6 modules
if (typeof window !== 'undefined') {
  window.PhysicsEngine = PhysicsEngine;
}

// Handle window resize
window.addEventListener("resize", function () {
  // Always resize canvas, even if physics isn't active
  // This ensures the canvas is ready when physics starts
  if (PhysicsEngine && typeof PhysicsEngine.resizeCanvas === 'function') {
    console.log("Window resized, resizing canvas and adjusting physics");
    PhysicsEngine.resizeCanvas();
  }
});
