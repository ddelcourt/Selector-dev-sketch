...
var PhysicsEngine = (function () {
  ...
  var enableVelocityThreshold = true; // Control whether to stop slow balls
  ...
  function init() {
    ...
    // TRACKING: Store pre-collision velocities for energy tracking
    var preCollisionData = new Map();
    var stuckBallDetection = new Map(); // Track balls stuck in continuous collision
    
    Events.on(engine, "collisionStart", function (event) {
      if (!enableVelocityThreshold) { // Only in zero-gravity mode
        event.pairs.forEach(function (pair) {
          var ball = null;
          var wall = null;
          
          // Identify ball and wall
          if (pair.bodyA.isStatic && !pair.bodyB.isStatic) {
            wall = pair.bodyA;
            ball = pair.bodyB;
          } else if (pair.bodyB.isStatic && !pair.bodyA.isStatic) {
            wall = pair.bodyB;
            ball = pair.bodyA;
          }
          
          // Only handle ball-wall collisions
          if (!ball || !wall) return;
          
          // Store pre-collision speed for this ball
          var speed = Math.sqrt(ball.velocity.x ** 2 + ball.velocity.y ** 2);
          preCollisionData.set(ball.id, speed);
          
          // Initialize stuck detection counter
          stuckBallDetection.set(ball.id, 0);
        });
      }
    });

    // TRACKING: Log ball[0] at collision end and restore energy
    Events.on(engine, "collisionEnd", function (event) {
      if (!enableVelocityThreshold) { // Only in zero-gravity mode
        event.pairs.forEach(function (pair) {
          ...
          var preSpeed = preCollisionData.get(ball.id);
          if (preSpeed !== undefined) {
            var currentSpeed = Math.sqrt(ball.velocity.x ** 2 + ball.velocity.y ** 2);
            
            // If speed decreased significantly, restore it (perfect elastic collision)
            if (currentSpeed > 0.001 && currentSpeed < preSpeed * 0.99) {
              var scale = preSpeed / currentSpeed;
            ...
            preCollisionData.delete(ball.id);
          }
          
          // Clear stuck detection counter
          stuckBallDetection.delete(ball.id);
        });
      }
    });

    // Detect and fix stuck/sliding balls in continuous collision
    Events.on(engine, "collisionActive", function (event) {
      if (!enableVelocityThreshold) { // Only in zero-gravity mode
        event.pairs.forEach(function (pair) {
          ...
          var stuckCount = stuckBallDetection.get(ball.id) || 0;
          stuckCount++;
          stuckBallDetection.set(ball.id, stuckCount);
          
          // If ball has been stuck for more than 5 frames, it's sliding (increased threshold)
          if (stuckCount === 6) { // Only trigger ONCE on frame 6
            var speed = Math.sqrt(ball.velocity.x ** 2 + ball.velocity.y ** 2);
            
            // Get collision normal (perpendicular to wall)
            var normal = pair.collision.normal;
            
            // Calculate velocity component along the normal
            var normalVelocity = ball.velocity.x * normal.x + ball.velocity.y * normal.y;
            
            // If ball is sliding (moving along wall, not into it)
            if (Math.abs(normalVelocity) < 0.15 && speed > 0.01) {
              var depth = pair.collision.depth || 0;
              
              // FIRST: Physically separate the ball from the wall
              var separationDistance = Math.max(depth + 5, 10); // Move at least 5 pixels beyond penetration depth
              Body.setPosition(ball, {
                x: ball.position.x + normal.x * separationDistance,
                y: ball.position.y + normal.y * separationDistance
              });
              
              // SECOND: Reflect velocity along normal (proper bounce physics)
              // Formula: v_reflected = v - 2(v·n)n
              var vDotN = ball.velocity.x * normal.x + ball.velocity.y * normal.y;
              var reflectedVx = ball.velocity.x - 2 * vDotN * normal.x;
              var reflectedVy = ball.velocity.y - 2 * vDotN * normal.y;
              
              // Apply reflected velocity with restitution
              Body.setVelocity(ball, {
                x: reflectedVx,
                y: reflectedVy
              });
    ...
    // (disabled during zero-gravity perpetual motion)
    Events.on(engine, "afterUpdate", function () {
      // In zero-gravity mode, cap maximum velocity to prevent wall tunneling
      if (!enableVelocityThreshold) {
        var maxSpeed = 5; // Maximum speed in zero-gravity mode
        balls.forEach(function (ball) {
          ...
          // If ball is moving too fast, scale velocity down
          if (speed > maxSpeed) {
        ...
        return; // Skip velocity threshold logic below
      }
      
      // Only apply threshold if enabled (not in zero-gravity mode)
      if (!enableVelocityThreshold) return;
  ...
  // Enable/disable velocity threshold (disable for perpetual motion)
  function setVelocityThreshold(enabled) {
    enableVelocityThreshold = enabled;
    console.log("Velocity threshold", enabled ? "enabled" : "disabled");
    
    // Also disable/enable Matter.js sleeping feature on all balls
    balls.forEach(function(ball) {
      // Force wake up and reset sleeping state
      Body.set(ball, { 
        isSleeping: false,
        sleepThreshold: enabled ? 60 : Infinity, // Infinity = never sleep
        timeScale: 1.0 // Ensure normal time scale
      });
      
      // If disabling threshold (perpetual motion), ensure ball is active
      if (!enabled) {
        // Give a tiny nudge if velocity is exactly zero to prevent stuck state
    ...
    // Disable engine sleeping globally when disabled
    if (engine) {
      engine.enableSleeping = enabled;
    }
  ...
  function diagnosticCheck() {
    ...
    console.log("State:");
    console.log("  - enableVelocityThreshold:", enableVelocityThreshold);
  ...
  return {
    ...
    setVelocityThreshold: setVelocityThreshold,
