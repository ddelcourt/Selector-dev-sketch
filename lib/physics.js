...
var PhysicsEngine = (function () {
  ...
  function init() {
    ...
    // Reduce collision slop to minimize energy loss in collisions
    engine.positionIterations = 10; // More iterations = more accurate
    ...
    // Store pre-collision velocities to preserve energy in zero-gravity mode
    var preCollisionSpeeds = new Map();
    var collisionLogCount = 0; // Limit console spam
    
    Events.on(engine, "collisionStart", function (event) {
      // Debug: Log first few collisions to verify events are firing
      if (collisionLogCount < 5) {
        console.log("Collision detected! enableVelocityThreshold:", enableVelocityThreshold, 
                    "pairs:", event.pairs.length);
        collisionLogCount++;
      }
      
      // Only preserve energy when velocity threshold is disabled (zero-gravity mode)
      if (enableVelocityThreshold) return;
      
      var pairs = event.pairs;
      pairs.forEach(function (pair) {
        // Only store speed for ball-to-wall collisions (one body is static)
        // For ball-to-ball collisions, Matter.js handles momentum conservation
        if (pair.bodyA.isStatic && !pair.bodyB.isStatic) {
          var speed = Math.sqrt(pair.bodyB.velocity.x ** 2 + pair.bodyB.velocity.y ** 2);
          preCollisionSpeeds.set(pair.bodyB.id, speed);
          if (collisionLogCount < 5) {
            console.log("  Ball-wall collision: ball speed =", speed.toFixed(3));
          }
        } else if (pair.bodyB.isStatic && !pair.bodyA.isStatic) {
          var speed = Math.sqrt(pair.bodyA.velocity.x ** 2 + pair.bodyA.velocity.y ** 2);
          preCollisionSpeeds.set(pair.bodyA.id, speed);
          if (collisionLogCount < 5) {
            console.log("  Ball-wall collision: ball speed =", speed.toFixed(3));
          }
        } else if (!pair.bodyA.isStatic && !pair.bodyB.isStatic) {
          if (collisionLogCount < 5) {
            console.log("  Ball-ball collision (not correcting)");
          }
        }
      });
    });
    ...
    Events.on(engine, "collisionEnd", function (event) {
      ...
      var pairs = event.pairs;
      pairs.forEach(function (pair) {
        // Only correct ball-to-wall collisions
        [pair.bodyA, pair.bodyB].forEach(function (body) {
          if (body.isStatic) return; // Skip walls
          
          var preSpeed = preCollisionSpeeds.get(body.id);
          if (preSpeed === undefined) return; // Only correct stored speeds
          
          var currentSpeed = Math.sqrt(body.velocity.x ** 2 + body.velocity.y ** 2);
          
          // If speed changed (due to numerical errors), restore it
          if (currentSpeed > 0.001 && Math.abs(currentSpeed - preSpeed) > 0.001) {
            var scale = preSpeed / currentSpeed;
            Body.setVelocity(body, {
              x: body.velocity.x * scale,
              y: body.velocity.y * scale
            });
            console.log("⚡ Corrected ball-wall: speed", currentSpeed.toFixed(3), "→", preSpeed.toFixed(3));
          }
          
          preCollisionSpeeds.delete(body.id);
        });
      });
    });
  ...
  function setFriction(frictionValue, frictionAirValue) {
    if (!engine || balls.length === 0) {
      console.warn("Cannot set friction: engine not initialized or no balls");
      return false;
    }
    ...
    console.log("Friction set to:", frictionValue, "frictionAir:", frictionAirValue, 
                "on", ballCount, "balls and", wallCount, "walls");
    console.log("Ball[0] friction check:", balls[0] ? 
                "friction=" + balls[0].friction + ", frictionAir=" + balls[0].frictionAir + 
                ", frictionStatic=" + balls[0].frictionStatic : "no balls");
    return true;
  }
  ...
  function setBounciness(bouncinessValue) {
    ...
    if (walls && walls.length > 0) {
      ...
      console.log("Bounciness set to:", bouncinessValue, "on", balls.length, "balls and", walls.length, "walls (1.0 = perfect elasticity)");
    } else {
  ...
  function setRandomVelocities(velocityRange) {
    ...
    balls.forEach(function(ball) {
      ...
      Body.setVelocity(ball, { x: vx, y: vy });
      console.log("Ball randomized: vx=" + vx.toFixed(2) + ", vy=" + vy.toFixed(2));
    });
  ...
  function setVelocityThreshold(enabled) {
    ...
    console.log("Velocity threshold", enabled ? "enabled" : "disabled", "(balls will", enabled ? "stop when slow" : "never stop" + ")");
    
    // Also disable/enable Matter.js sleeping feature on all balls
    balls.forEach(function(ball) {
      ball.isSleeping = false; // Wake up any sleeping balls
      Body.set(ball, { 
        isSleeping: false,
    ...
    if (engine) {
      ...
      console.log("Engine sleeping globally", enabled ? "enabled" : "disabled");
    }
    
    console.log("Matter.js sleeping", enabled ? "enabled" : "disabled", "on", balls.length, "balls");
    return true;
  }
