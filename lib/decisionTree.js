  ...
  function initializeUI(tree) {
    ...
    if (typeof PhysicsEngine !== "undefined") {
      ...
      setTimeout(function() {
        ...
        // IMPORTANT: Ensure physics starts with normal settings regardless of saved config
        PhysicsEngine.setGravity(1);
        PhysicsEngine.setFriction(0.05, 0.01);
        PhysicsEngine.setBounciness(0.95);
        console.log("Physics initialized with normal gravity (1.0), friction, and bounciness (0.95)");
      ...
      try {
        ...
        if (kids.length > 0 && !hasOnlyEndChild) {
          ...
          if (physicsMode && typeof PhysicsEngine !== "undefined") {
            ...
            // After 3 seconds: Invert gravity (very gentle) + randomize velocities
            setTimeout(function() {
              PhysicsEngine.setGravity(-0.04); // -1 / 25 (divided by 5, then by 5 again)
              PhysicsEngine.setRandomVelocities({ min: -2, max: 2 }); // Gentle scatter
              console.log("Gravity inverted at -0.04 (very gentle upward float) + velocities randomized");
            }, 3000);
            
            // After 6 seconds: Zero gravity + perfect elasticity (perpetual motion)
            setTimeout(function() {
              PhysicsEngine.setGravity(0);
              PhysicsEngine.setFriction(0, 0); // No friction, no air resistance
              PhysicsEngine.setBounciness(1.0); // Perfect elasticity - no energy loss
              PhysicsEngine.setRandomVelocities({ min: -3, max: 3 }); // Random scatter in all directions
              PhysicsEngine.setVelocityThreshold(false); // Disable stopping slow balls + disable sleeping
              console.log("PERPETUAL MOTION: Zero gravity + zero friction + bounciness 1.0 + sleeping disabled");
    ...
    $("#back").on("click", function (e) {
      ...
      // Reset gravity, friction, and bounciness to normal if in physics mode
      if (physicsMode && typeof PhysicsEngine !== "undefined") {
        PhysicsEngine.setGravity(1);
        PhysicsEngine.setFriction(0.05, 0.01); // Restore normal friction and air resistance
        PhysicsEngine.setBounciness(0.95); // Restore normal bounciness
        PhysicsEngine.setVelocityThreshold(true); // Re-enable velocity threshold
        console.log("Gravity, friction, bounciness, and velocity threshold reset to normal");
      }
    ...
    $("#restart").on("click", function (e) {
      ...
      if (physicsMode && typeof PhysicsEngine !== "undefined") {
        PhysicsEngine.clearBalls();
        // Reset gravity, friction, and bounciness to normal
        PhysicsEngine.setGravity(1);
        PhysicsEngine.setFriction(0.05, 0.01); // Restore normal friction and air resistance
        PhysicsEngine.setBounciness(0.95); // Restore normal bounciness
        PhysicsEngine.setVelocityThreshold(true); // Re-enable velocity threshold
