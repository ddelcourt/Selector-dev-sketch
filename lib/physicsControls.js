...
(function() {
  ...
  // Default configuration values (aligned with physics.js)
  const defaults = {
  ...
  // Apply a configuration to all UI elements, physics engine, and localStorage
  function applyConfig(config) {
    console.log('Applying configuration:', config);
    
    // Validate and merge with defaults
    const fullConfig = Object.assign({}, defaults, config);
    
    // Update all slider values
    document.getElementById('ballSize').value = fullConfig.ballSizePercent;
    document.getElementById('velocityXMin').value = fullConfig.velocityX.min;
    document.getElementById('velocityXMax').value = fullConfig.velocityX.max;
    document.getElementById('velocityYMin').value = fullConfig.velocityY.min;
    document.getElementById('velocityYMax').value = fullConfig.velocityY.max;
    document.getElementById('bounciness').value = fullConfig.bounciness;
    document.getElementById('friction').value = fullConfig.friction;
    document.getElementById('gravity').value = fullConfig.gravity;
    document.getElementById('textSize').value = fullConfig.textSizeFactor;
    document.getElementById('velocityThreshold').value = fullConfig.velocityThreshold;
    
    // Update all checkbox values
    document.getElementById('allCaps').checked = fullConfig.allCaps;
    document.getElementById('whiteText').checked = fullConfig.whiteText;
    document.getElementById('enableWindowShake').checked = fullConfig.enableWindowShake;
    document.getElementById('enablePulseEffect').checked = fullConfig.enablePulseEffect;
    document.getElementById('randomTopBottom').checked = fullConfig.randomTopBottom;
    
    // Update all display values
    const ballSizePercent = (fullConfig.ballSizePercent * 100).toFixed(2);
    document.getElementById('ballSizeValue').textContent = ballSizePercent + '%';
    document.getElementById('velocityXMinValue').textContent = fullConfig.velocityX.min.toFixed(1);
    document.getElementById('velocityXMaxValue').textContent = fullConfig.velocityX.max.toFixed(1);
    document.getElementById('velocityYMinValue').textContent = fullConfig.velocityY.min.toFixed(1);
    document.getElementById('velocityYMaxValue').textContent = fullConfig.velocityY.max.toFixed(1);
    document.getElementById('bouncinessValue').textContent = fullConfig.bounciness.toFixed(2);
    document.getElementById('frictionValue').textContent = fullConfig.friction.toFixed(2);
    document.getElementById('gravityValue').textContent = fullConfig.gravity.toFixed(1);
    document.getElementById('textSizeValue').textContent = fullConfig.textSizeFactor.toFixed(2);
    document.getElementById('velocityThresholdValue').textContent = fullConfig.velocityThreshold.toFixed(2);
    
    // Update physics engine
    if (typeof PhysicsEngine !== 'undefined') {
      PhysicsEngine.setConfig(fullConfig);
    }
  ...
  function initControls() {
    ...
    document.getElementById('whiteText').checked = savedConfig.whiteText !== undefined ? savedConfig.whiteText : defaults.whiteText;
    ...
    document.getElementById('enableWindowShake').checked = savedConfig.enableWindowShake !== undefined ? savedConfig.enableWindowShake : defaults.enableWindowShake;
    document.getElementById('enablePulseEffect').checked = savedConfig.enablePulseEffect !== undefined ? savedConfig.enablePulseEffect : defaults.enablePulseEffect;
    document.getElementById('randomTopBottom').checked = savedConfig.randomTopBottom !== undefined ? savedConfig.randomTopBottom : defaults.randomTopBottom;
    ...
    // White Text checkbox
    const whiteTextCheckbox = document.getElementById('whiteText');
    if (whiteTextCheckbox) {
      whiteTextCheckbox.addEventListener('change', function() {
        PhysicsEngine.setConfig({ whiteText: whiteTextCheckbox.checked });
        saveConfig(PhysicsEngine.getConfig());
      });
    }

    // Enable Window Shake checkbox
    const enableWindowShakeCheckbox = document.getElementById('enableWindowShake');
    if (enableWindowShakeCheckbox) {
      enableWindowShakeCheckbox.addEventListener('change', function() {
        PhysicsEngine.setConfig({ enableWindowShake: enableWindowShakeCheckbox.checked });
        saveConfig(PhysicsEngine.getConfig());
      });
    }

    // Enable Pulse Effect checkbox
    const enablePulseEffectCheckbox = document.getElementById('enablePulseEffect');
    if (enablePulseEffectCheckbox) {
      enablePulseEffectCheckbox.addEventListener('change', function() {
        PhysicsEngine.setConfig({ enablePulseEffect: enablePulseEffectCheckbox.checked });
        saveConfig(PhysicsEngine.getConfig());
      });
    }

    // Random Top/Bottom Position checkbox
    const randomTopBottomCheckbox = document.getElementById('randomTopBottom');
    if (randomTopBottomCheckbox) {
      randomTopBottomCheckbox.addEventListener('change', function() {
        PhysicsEngine.setConfig({ randomTopBottom: randomTopBottomCheckbox.checked });
        saveConfig(PhysicsEngine.getConfig());
      });
    }
    ...
    if (presetSelector) {
      presetSelector.addEventListener('change', async function() {
        ...
        try {
          ...
          console.log(`Loading preset: ${presetConfig.name}`, presetConfig);
          applyConfig(presetConfig);
          alert(`Preset "${presetConfig.name}" loaded successfully!`);
        } catch (error) {
    ...
    if (uploadSettings) {
      uploadSettings.addEventListener('change', function(event) {
        ...
        if (!file.name.endsWith('.json')) {
          ...
          uploadSettings.value = ''; // Reset input
          return;
        }
        ...
        reader.onload = function(e) {
          try {
            ...
            console.log('Uploaded configuration:', uploadedConfig);
            
            // Validate required properties
            if (typeof uploadedConfig.ballSizePercent === 'undefined') {
              throw new Error('Invalid configuration file: missing required properties');
            }
            
            applyConfig(uploadedConfig);
  ...
  function resetToDefaults() {
    ...
    // Also reset the preset selector
    const presetSelector = document.getElementById('presetSelector');
    if (presetSelector) {
  ...
  function exportSettings() {
    ...
    if (typeof PhysicsEngine !== 'undefined' && PhysicsEngine.getConfig) {
      ...
      currentConfig = {
        ...
        enableWindowShake: document.getElementById('enableWindowShake').checked,
        enablePulseEffect: document.getElementById('enablePulseEffect').checked,
        randomTopBottom: document.getElementById('randomTopBottom').checked
      };
    }
