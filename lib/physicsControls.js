...
(function() {
  ...
  function initControls() {
    ...
    document.getElementById('canvasBorder').checked = savedConfig.canvasBorder !== undefined ? savedConfig.canvasBorder : defaults.canvasBorder;
    ...
    // Canvas Border checkbox
    const canvasBorderCheckbox = document.getElementById('canvasBorder');
    if (canvasBorderCheckbox) {
      const canvas = document.getElementById('physicsCanvas');
      // Apply initial state
      if (canvas) {
        canvas.style.border = canvasBorderCheckbox.checked ? '1px solid red' : 'none';
      }
      canvasBorderCheckbox.addEventListener('change', function() {
        if (canvas) {
          canvas.style.border = canvasBorderCheckbox.checked ? '1px solid red' : 'none';
        }
        const config = PhysicsEngine.getConfig();
        config.canvasBorder = canvasBorderCheckbox.checked;
        saveConfig(config);
      });
    }
